...
simulation_state = {
    "current_green": SIMULATION_SEQUENCE[0][0],
...
def simulation_loop():
    while running:
        with state_lock:
            remaining = simulation_state["remaining_time"]
            if remaining <= 0:
...
def active_phase_for_direction(direction, phase):
    return phase == direction or phase == f"YELLOW_{direction}"


def gen_simulated_frames(direction):
    ...
    while running:
        ...
        if active_phase_for_direction(direction, phase):
...
@demo_bp.route("/video/<direction>")
def demo_video_feed(direction):
    direction = direction.upper()
    if direction not in camera_streams:
        abort(404)
    return Response(gen_simulated_frames(direction), mimetype="multipart/x-mixed-replace; boundary=frame")
